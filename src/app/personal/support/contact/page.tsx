  ...
  return (
    <div className="bg-white text-slate-900 font-manrope min-h-screen">
        ...
        <div className="max-w-[1440px] mx-auto px-4 sm:px-10">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {contactOptions.map((opt, idx) => {
              ...
              return (
                <div key={idx} className="p-8 rounded-2xl bg-slate-50 border border-slate-200 space-y-3 flex flex-col justify-between">
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <div className="w-10 h-10 rounded-xl bg-blue-50 text-blue-600 flex items-center justify-center">
                        <IconComp className="w-5 h-5" />
                      </div>
                      <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider bg-slate-200/60 px-2 py-0.5 rounded-full">
                        {opt.badge}
                      </span>
                    </div>

                    <h3 className="text-xl font-bold text-slate-900">{opt.title}</h3>
                    <p className="text-sm text-slate-700 font-medium">{opt.desc}</p>
                  </div>
      ...
      <section className="py-20 bg-slate-50">
        <div className="max-w-[1440px] mx-auto px-4 sm:px-10">
          <div className="max-w-2xl mx-auto p-8 sm:p-12 rounded-3xl bg-white border border-slate-200 shadow-xl space-y-8">
            {!submitted ? (
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="w-full px-4 py-3 rounded-xl border border-slate-300 text-sm font-medium text-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-600 bg-white"
                    >
                ...
                <div className="flex items-start gap-3 pt-2">
                  ...
                  <label htmlFor="consent" className="text-xs text-slate-600 cursor-pointer leading-relaxed">
                    I consent to Treel processing my support inquiry. See <Link href="/privacy" className="text-blue-600 underline">Privacy Policy</Link>.
                  </label>
                </div>

                <button
                  type="submit"
                  className="w-full py-4 rounded-xl bg-blue-600 text-white font-bold text-sm hover:bg-blue-700 transition-all shadow-md cursor-pointer flex items-center justify-center gap-2"
                >
              ...
              <div className="p-8 rounded-3xl bg-slate-50 border border-slate-200 text-center space-y-4">
                ...
                <h3 className="text-2xl font-bold text-slate-900">Support ticket submitted.</h3>
