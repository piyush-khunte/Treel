      ...
      {!isOpen && (
        <button
          ...
          className={`group flex items-center gap-2.5 p-3.5 sm:px-5 sm:py-3.5 rounded-full shadow-2xl transition-all duration-300 hover:scale-105 cursor-pointer ${
            isTmip
              ? "bg-[#0F1419] text-[#FAF7F2] hover:bg-[#1A2330] border border-[#D5573B]"
              : "bg-[#0F1E19] text-[#FAF7F2] hover:bg-[#14532D] border border-[#00B37E]/30"
          }`}
          aria-label="Open AI assistant"
        >
          <div className="relative">
            <Bot className={`w-5 h-5 ${isTmip ? "text-[#D5573B]" : "text-[#00B37E]"}`} />
            <span className={`absolute -top-1 -right-1 w-2.5 h-2.5 rounded-full animate-ping ${isTmip ? "bg-[#D5573B]" : "bg-[#00E5FF]"}`} />
            <span className={`absolute -top-1 -right-1 w-2.5 h-2.5 rounded-full ${isTmip ? "bg-[#D5573B]" : "bg-[#00E5FF]"}`} />
          </div>
      ...
      {isOpen && (
        <div className="w-[350px] sm:w-[390px] h-[520px] bg-white rounded-3xl shadow-2xl border border-slate-200/80 flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
          ...
          <div className={`p-4 flex items-center justify-between ${
            isTmip
              ? "bg-[#0F1419] text-[#FAF7F2] border-b border-[#D5573B]/30"
              : "bg-[#0F1E19] text-white border-b border-[#14532D]"
          }`}>
            <div className="flex items-center gap-3">
              <div className={`w-9 h-9 rounded-full flex items-center justify-center ${
                isTmip ? "bg-[#D5573B]/20 text-[#D5573B]" : "bg-[#00B37E]/20 text-[#00B37E]"
              }`}>
                <Bot className="w-5 h-5" />
              </div>
              <div>
                <div className="text-sm font-semibold flex items-center gap-1.5">
                  Magic Flow AI <Sparkles className={`w-3.5 h-3.5 ${isTmip ? "text-[#D5573B]" : "text-[#00E5FF]"}`} />
                </div>
                <div className="text-[11px] text-slate-300 flex items-center gap-1">
                  <span className={`w-2 h-2 rounded-full ${isTmip ? "bg-[#D5573B]" : "bg-[#00E599]"}`} /> Online · Treel Assistant
                </div>
              </div>
            </div>
            <button
              ...
              >
                <div
                  className={`max-w-[82%] p-3.5 rounded-2xl leading-relaxed ${
                    m.sender === "user"
                      ? isTmip
                        ? "bg-[#0F1419] text-white rounded-br-none shadow-sm"
                        : "bg-[#0F1E19] text-white rounded-br-none shadow-sm"
          ...
          <div className="px-3 py-2 bg-white border-t border-slate-100 flex items-center gap-1.5 overflow-x-auto no-scrollbar text-[11px]">
            <button
              ...
              className={`px-2.5 py-1 rounded-full whitespace-nowrap ${
                isTmip ? "bg-slate-100 hover:bg-[#D5573B]/10 hover:text-[#D5573B] text-slate-700" : "bg-slate-100 hover:bg-slate-200 text-slate-700"
              }`}
            >
              TMIP Fleet
            </button>
            <button
              ...
              className="px-2.5 py-1 rounded-full bg-amber-50 hover:bg-amber-100 text-amber-800 whitespace-nowrap"
            >
              Suraksha ROI
            </button>
            <button
              onClick={() => setInputText("What is the price of Personal TPMS?")}
              className="px-2.5 py-1 rounded-full bg-blue-50 hover:bg-blue-100 text-blue-700 whitespace-nowrap"
            >
              Buy TPMS
            </button>
          </div>
          ...
          <form onSubmit={handleSend} className="p-3 bg-white border-t border-slate-200 flex gap-2">
            <input
              ...
              className={`flex-1 text-xs px-3.5 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 bg-slate-50 ${
                isTmip ? "focus:ring-[#D5573B]" : "focus:ring-[#00B37E]"
              }`}
            />
            {isTmip ? (
              <button
                type="submit"
                className="h-10 w-10 p-0 shrink-0 rounded-xl bg-[#D5573B] hover:bg-[#CB4831] text-[#FAF7F2] flex items-center justify-center transition-colors shadow-sm"
              >
                <Send className="w-4 h-4" />
              </button>
            ) : (
              <Button type="submit" variant="emerald" size="sm" className="h-10 w-10 p-0 shrink-0 rounded-xl">
                <Send className="w-4 h-4" />
              </Button>
            )}
          </form>
